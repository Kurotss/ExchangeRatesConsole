...
namespace ExchangeRatesConsole.Database
{
    ...
    public class ExchangeRateProcedures
    {
        ...
        public void InsertExchangeRate(Valute exchangeRate, DateTime date)
        {
            try
            {
                if (!decimal.TryParse(exchangeRate.Value.Replace(",", CultureInfo.InvariantCulture.NumberFormat.NumberDecimalSeparator), CultureInfo.InvariantCulture, out decimal value))
                    throw new ArgumentException($"Некорректное значение Value: {exchangeRate.Value}");
                if (!decimal.TryParse(exchangeRate.VunitRate.Replace(",", CultureInfo.InvariantCulture.NumberFormat.NumberDecimalSeparator), CultureInfo.InvariantCulture, out decimal vunitRate))
